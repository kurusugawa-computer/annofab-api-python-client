...
def test_get_project_member_from_account_id__存在するaccount_idならプロジェクトメンバを返す():
    member = {
        "project_id": PROJECT_ID,
        "account_id": "account_id",
        "user_id": "user_id",
    }
    repository = create_repository_with_members([member])

    assert repository.get_project_member_from_account_id(PROJECT_ID, "account_id") == member


def test_get_project_member_from_account_id__存在しないaccount_idならValueErrorを送出する():
    repository = create_repository_with_members([])

    with pytest.raises(ValueError):
        repository.get_project_member_from_account_id(PROJECT_ID, "unknown_account_id")
...
def test_get_project_member_from_user_id__存在するuser_idならプロジェクトメンバを返す():
    member = {
        "project_id": PROJECT_ID,
        "account_id": "account_id",
        "user_id": "user_id",
    ...
    assert repository.get_project_member_from_user_id(PROJECT_ID, "user_id") == member


def test_get_project_member_from_user_id__存在しないuser_idならValueErrorを送出する():
    repository = create_repository_with_members([])

    with pytest.raises(ValueError):
        repository.get_project_member_from_user_id(PROJECT_ID, "unknown_user_id")
