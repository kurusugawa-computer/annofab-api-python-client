...
class RegisterAnnotation:
    ...
    def get_annotations_for_editor(self, project_id: str, task_id: str,
        ...
        return self.service.api._request_wrapper(http_method, url_path,
                                                 **keyword_params)
    ...
    def update_annotation_with_image(self, project_id: str, task_id: str,
                                     input_data_id: str,
        ...
        old_annotations = self.get_annotations_for_editor(
            project_id, task_id, input_data_id)[0]
        ...
        for e in image_file_list:
            ...
            s3_path = self.service.wrapper.upload_file_to_s3(
                project_id, image_file, "image/png")
        ...
        return self.service.api.put_annotation(project_id,
                                               task_id,
                                               input_data_id,
                                               request_body=request_body)[0]

    def write_segmentation_image(self, input_data: Dict[str, Any], label: str,
    ...
    def write_segmentation_image_for_labels(
            self, labels: List[Dict[str, str]], input_data: Dict[str, Any],
    ...
    def register_raster_annotation_from_polygon(
            self, annotation_dir: str, default_input_data_size: InputDataSize,
            ...
            for input_data_json_path in task_dir.iterdir():
                ...
                try:
                    image_file_list = self.write_segmentation_image_for_labels(
                        labels, input_data, default_input_data_size,
                        tmp_image_dir)
                ...
                try:
                    ...
                    if task["account_id"] == account_id and task[
                        ...
                        self.facade.change_operator_of_task(
                            project_id, task_id, account_id)
                        self.facade.change_to_working_phase(
                            project_id, task_id, account_id)
                except Exception as e:
                    logger.warning(e)
                    logger.warning(
