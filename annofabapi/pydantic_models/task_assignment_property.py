...
from annofabapi.pydantic_models.task_assignment_order import TaskAssignmentOrder


class TaskAssignmentProperty(BaseModel):
    """
    プロジェクト設定でタスクプロパティ割当を有効にしている場合のみ指定してください。
    """

    name: StrictStr = Field(
        description="タスクの割当優先度を決定するためのタスクプロパティ。  指定できるプロパティは次のいずれかです。  * `task_id` * `updated_datetime` * `metadata` (`metadata.{メタデータ名}` の形式で指定してください) "
    )
