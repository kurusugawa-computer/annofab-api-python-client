...
class AnnotationTypeFieldDefinitionOneBooleanField(BaseModel):
    """
    ...
    def type_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(["OneBooleanField"]):
            raise ValueError("must be one of enum values ('OneBooleanField')")
        return value
        ...
        _obj = cls.model_validate(
            {
                ...
                "label": InternationalizationMessage.from_dict(obj["label"]) if obj.get("label") is not None else None,
            }
        )
        return _obj
