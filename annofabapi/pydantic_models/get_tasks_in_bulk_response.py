...
from annofabapi.pydantic_models.task_identifier import TaskIdentifier


class GetTasksInBulkResponse(BaseModel):
    """
    複数のタスク取得時のレスポンス
    """

    success: List[Task] = Field(description="取得に成功したタスク")
    failure: List[TaskIdentifier] = Field(description="取得に失敗したタスクのIDとそのプロジェクトID")
    ...
    def from_dict(cls, obj: Dict[str, Any] | None) -> Self | None:
        ...
        _obj = cls.model_validate(
            {
                "success": [Task.from_dict(_item) for _item in obj["success"]] if obj.get("success") is not None else None,
                "failure": [TaskIdentifier.from_dict(_item) for _item in obj["failure"]] if obj.get("failure") is not None else None,
            }
        )
        return _obj
