...
class AnnotationTypeFieldDefinitionOneIntegerField(BaseModel):
    """
    ...
    initial_value: Optional[StrictInt] = Field(default=None, description="フィールドの初期値 ")
    __properties: ClassVar[List[str]] = ["_type", "title", "prefix", "postfix", "description", "initial_value"]

    @field_validator("type")
    def type_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(["OneIntegerField"]):
            raise ValueError("must be one of enum values ('OneIntegerField')")
        return value
