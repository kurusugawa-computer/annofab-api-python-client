...
class AbstractAnnofabApi(abc.ABC):
    ...
    def get_project_comments_url(self, project_id: str, **kwargs) -> Tuple[Any, requests.Response]:
        ...
        コメント全件ファイルにアクセスするための、認証済み一時URLを取得します。 取得したURLは1時間で失効し、アクセスできなくなります。  ### コメント全件ファイル コメント全件ファイルには、プロジェクトのコメント情報がJSON形式（[Comment](#section/Comment)の配列）で記録されています。 このファイルは毎日AM 02:00 JSTに更新されます。
